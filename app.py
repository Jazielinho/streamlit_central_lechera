...
if len(text_info_df) > 10000:
    text_info_df = text_info_df.sample(frac=0.1, random_state=123).reset_index(drop=True)
...
tab1, tab2, tab5, tab3, tab4, tab6 = st.tabs(['GENERAL', 'TOPICOS', 'AUTORES', 'PERSONAS MENCIONADAS', 'HASHTAGS', 'IMAGES'])
...
with tab2:
    ...
    #     _x = selected_point['x']
    #     _y = selected_point['y']
    #     _filter_df = text_info_df[(text_info_df['x'] == _x) & (text_info_df['y'] == _y)]
    #     # print(_filter_df)
    #     _img_url = _filter_df['image_url'].values
    #     if len(_img_url) > 0:
    #         _img_url = _img_url[0]
    #         print(_img_url)
    #         try:
    #             st.image(_img_url, use_column_width=True)
...
with tab5:
    st.markdown(f'''## Autores con más textos ''')
    count_autores_df = text_info_df['user_username'].value_counts().reset_index()
    count_autores_df.columns = ['autor', 'count']
    _count_autores_df = count_autores_df.head(30)
    fig = make_subplots(specs=[[{"secondary_y": True}]])
    fig.add_trace(go.Scatter(x=_count_autores_df['autor'], y=_count_autores_df['count'], name='Autores con más texto', mode='lines+markers'), secondary_y=True)
    fig.update_layout(barmode='stack', legend=dict(orientation="h", yanchor="bottom", y=1.02, xanchor="right", x=1), title_text='Cantidad de textos')
    fig.update_layout(width=WIDTH, height=HEIGHT)
    st.plotly_chart(fig, use_container_width=True)

    autores_seleccionados = st.multiselect('Elige autor', count_autores_df['autor'], count_autores_df['autor'][:2])

    if st.button('Analizar texto de autores mencionadas'):
        for autor_elegido in autores_seleccionados:
            autor_text_df = text_info_df[text_info_df['user_username'] == autor_elegido]

            st.markdown(f'''#### Autor: {autor_elegido}''')

            st.markdown(f'''##### Estadísticas''')
            general_statistics_df = autor_text_df[['retweet_count', 'reply_count', 'like_count', 'quote_count', 'impression_count']].sum().astype(int).reset_index()
            general_statistics_df.columns = ['Estadística', 'Valor']
            st.table(general_statistics_df)

            statistics_date_df = autor_text_df.groupby('date')[['id', 'retweet_count', 'reply_count', 'like_count', 'quote_count', 'impression_count']].agg({'id': 'count', 'retweet_count': 'sum', 'reply_count': 'sum', 'like_count': 'sum', 'quote_count': 'sum','impression_count': 'sum'}).reset_index()
            statistics_date_df['engagement'] = statistics_date_df['retweet_count'] + statistics_date_df['reply_count'] + statistics_date_df['like_count'] + statistics_date_df['quote_count']
            statistics_date_df['engagement_rate'] = statistics_date_df['engagement'] / statistics_date_df['impression_count']
            ...
            st.markdown(f'''##### Sentimiento de los textos ''')
            general_sentiment_df = autor_text_df[['negative_score', 'neutral_score', 'positive_score']].mean().reset_index()
            general_sentiment_df.columns = ['Sentimiento', 'Valor']
            st.table(general_sentiment_df)

            sentiment_date_df = autor_text_df.groupby('date')[['negative_score', 'neutral_score', 'positive_score']].mean().reset_index()
            fig = make_subplots(specs=[[{"secondary_y": True}]])
            fig.add_trace(go.Bar(x=sentiment_date_df['date'], y=sentiment_date_df['negative_score'], name='Sentimiento negativo'), secondary_y=False)
            fig.add_trace(go.Bar(x=sentiment_date_df['date'], y=sentiment_date_df['neutral_score'], name='Sentimiento neutral'), secondary_y=False)
            fig.add_trace(go.Bar(x=sentiment_date_df['date'], y=sentiment_date_df['positive_score'], name='Sentimiento positivo'), secondary_y=False)
            ...
            topic_count_sentiment_df = autor_text_df.groupby('topic').agg({'id': 'count', 'negative_score': 'mean', 'neutral_score': 'mean', 'positive_score': 'mean'}).reset_index()
            topic_count_sentiment_df = topic_count_sentiment_df[topic_count_sentiment_df['topic'] != -1].sort_values('topic')
            topic_count_sentiment_df['topic_label'] = [dict_topic_labels[x] for x in topic_count_sentiment_df['topic']]
            fig = make_subplots(specs=[[{"secondary_y": True}]])
            fig.add_trace(go.Scatter(x=topic_count_sentiment_df['topic_label'], y=topic_count_sentiment_df['id'], name='Cantidad de textos', mode='lines+markers'), secondary_y=True)
            ...
            fig = go.Figure()
            _df = autor_text_df[autor_text_df['topic'] == -1]
            fig.add_trace(go.Scatter(x=_df['x'], y=_df['y'], hovertext=_df['content'], hoverinfo='text', mode='markers+text', name='Sin tópico', marker=dict(color='#CFD8DC', size=5, opacity=0.5), showlegend=False))
            all_topics = sorted(autor_text_df['topic'].unique())
            for topic in all_topics:
                if int(topic) == -1:
                    continue
                selection = autor_text_df[autor_text_df['topic'] == topic]
                fig.add_trace(go.Scatter(x=selection['x'], y=selection['y'], hovertext=selection['content'], hoverinfo='text', mode='markers+text', name=dict_topic_labels[topic], marker=dict(size=5, opacity=0.5)))
            x_range = [autor_text_df['x'].min() - abs(autor_text_df['x'].min() * 0.15), autor_text_df['x'].max() + abs(autor_text_df['x'].max() * 0.15)]
            y_range = [autor_text_df['y'].min() - abs(autor_text_df['y'].min() * 0.15), autor_text_df['y'].max() + abs(autor_text_df['y'].max() * 0.15)]
            fig.add_shape(type="rect", x0=sum(x_range) / 2, y0=y_range[0], x1=sum(x_range) / 2, y1=y_range[1], line=dict(color="#CFD8DC", width=2))
            fig.add_shape(type="rect", x0=x_range[0], y0=sum(y_range) / 2, x1=x_range[1], y1=sum(y_range) / 2, line=dict(color="#CFD8DC", width=2))
            fig.add_annotation(x=x_range[0], y=sum(y_range) / 2, text="D1", showarrow=False, yshift=10)
            fig.add_annotation(x=sum(x_range) / 2, y=y_range[1], text="D2", showarrow=False, xshift=10)
